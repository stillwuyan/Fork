...
int print_disc_status(const char* path) {
    ...
    printf("   Drive: ");
    int result = ioctl(fd, CDROM_DRIVE_STATUS, 0);
    switch (result) {
    case CDS_NO_INFO:
        printf("NO INFO\n");
        break;
    ...
    result = ioctl(fd, CDROM_DISC_STATUS, 0);
    printf("   Disc: ");
    switch (result) {
    case CDS_NO_INFO:
        printf("NO INFO\n");
...
int main (void)
{
	...
	udev_list_entry_foreach(dev_list_entry, devices) {
		...
		/* usb_device_get_devnode() returns the path to the device node
		   itself in /dev. */
		if (memcmp(udev_device_get_devnode(dev), "/dev/sr", 7) != 0) {
			dev_tmp = udev_device_unref(dev);
			continue;
		}
		...
		dev_parent = udev_device_get_parent_with_subsystem_devtype(
		       ...
		       "scsi_device");
		if (!dev_parent) {
			printf("Unable to find parent scsi device.\n");
			//exit(1);
		...
		printf("  Vendor/Model: %s %s\n",
		        udev_device_get_sysattr_value(dev_parent,"vendor"),
		        udev_device_get_sysattr_value(dev_parent, "model"));
		udev_device_unref(dev_parent);
		udev_device_unref(dev);
	}
	...
	while (1) {
		...
		if (ret > 0 && FD_ISSET(fd, &fds)) {
			...
			if (dev) {
				...
				print_disc_status(udev_device_get_devnode(dev));
				udev_device_unref(dev);
			}
			else {
